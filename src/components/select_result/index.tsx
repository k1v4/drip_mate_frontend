...
const SelectResult: React.FC = (): JSX.Element => {
  ...
  useEffect(() => {
    if (isDetailMode) {
      ...
      const fetchOutfit = async () => {
          try {
              const response = await instance.get(`/api/v1/users/me/outfits/${id}`);
    ...
    const handleSaveOutfit = async () => {
        ...
        try {
            setSaving(true);
            await instance.post('/api/v1/users/me/outfits', {
  ...
  const handleDeleteOutfit = async () => {
      try {
          await instance.delete(`/api/v1/users/me/outfits/${id}`);
          navigate('/');
      } catch (e) {
